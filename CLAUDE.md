...
| `scribble/` | Standalone scribble calendar (not Obsidian-linked): `MonthScribbleScreen` (single freeform ink layer over a month grid, plus a search bar), `MonthScribbleStore` (per-month on-device ink persistence in `filesDir/month-scribbles/`; pure-text serialize/deserialize + `scaleStrokes`), `MonthGridRenderer` (`drawMonthGrid` paints the grid into the ink surface via `MemoCanvas`'s `backgroundRenderer`; `weekRowsFor` row math), `ScribbleGrid` (pure cell geometry: `gridGeometry`/`dateAtPoint`/`bucketStrokesByDay`), `ScribbleIndex` (pure: searchable text sidecar serialize/deserialize, `searchIndex`, `inkHash`), `ScribbleIndexer` (Android glue: recognises each month into a `.idx` sidecar and searches them) |
...
| `update/` | In-app updater: `UpdateVersions` (pure semver compare), `GitHubUpdateClient` (reads the repo's latest release), `ApkInstaller` (download + system-installer handoff), `UpdateSettingsStore` (DataStore: last-check / auto-check), `UpdateManager` (orchestration + `UpdateUiState`), `UpdateBanner` (Calendar "Update available" card) |
...
**Searchable scribble handwriting**: the scribble ink is made searchable *without altering it* — recognition writes a text sidecar next to each month's `.ink` (`<YearMonth>.idx`), never touching the strokes. To get day-level results from one freeform layer, `bucketStrokesByDay` (pure, in `ScribbleGrid`) assigns each stroke to the day cell containing its bounding-box midpoint, reusing the exact grid geometry `drawMonthGrid` paints with (`gridGeometry`/`dateAtPoint`); ink outside the grid (title/notes area) becomes a month-level bucket. `ScribbleIndexer` (Android glue) recognises each bucket via the configured HWR engine at the screen's existing flush moments (leaving a month / closing the screen) plus a one-time backfill on open, skipping unchanged months by an `inkHash` of the `.ink` text and *not* writing an index if recognition is `Unavailable` (so it retries). The sidecar is the same compact pure-text format as `.ink` (`serializeIndex`/`deserializeIndex`): `day=<isoDate>\t<text>` lines plus a `free=` month-level line. The search bar runs the pure `searchIndex` over all `.idx` files (case-insensitive, newest-first) and jumps to a hit's month with the day cell outlined via `drawMonthGrid`'s `highlightDay` (cleared by writing or navigating). No network, no new permissions — recognition stays on-device.

**Handwriting → text recognition** goes entirely through the Onyx built-in MyScript engine (`OnyxHWREngine`). There are no cloud/AI recognition paths — all recognition stays on-device.

**Network use is limited to the in-app updater** (`update/`). The app is sideloaded from GitHub Releases, not a store, so it self-updates: on launch (throttled to once a day) `UpdateManager` asks `GitHubUpdateClient` for the repo's latest release, compares the tag to `BuildConfig.VERSION_NAME` (pure `UpdateVersions`), and shows `UpdateBanner` on the Calendar when newer. Tapping it downloads the signed APK and launches the system installer (`ApkInstaller` via a `FileProvider`) — same signing key, so it updates in place with no data loss; the install is always user-confirmed. This is the **only** network call the app makes, it's fail-silent (offline just hides the banner), and the public repo means no token is embedded. It adds the `INTERNET` and `REQUEST_INSTALL_PACKAGES` permissions. A "Check for updates now" button lives in Settings → Updates.
...
JVM unit tests (no Android runtime needed) cover parsers, formatters, `StrokeStore`, `EraseHitTest`, `DailyNoteRepository`, `DiagramPaths` (naming/folder/collision rules), `MonthScribbleStore` (serialization round-trip, month isolation, atomic replace, `scaleStrokes`), `weekRowsFor` (grid row math), `ScribbleGrid` (`dateAtPoint` cell mapping + `bucketStrokesByDay` stroke-to-day assignment), `ScribbleIndex` (sidecar serialize/deserialize round-trip, `inkHash` change detection, `searchIndex` case-insensitive ordering), `SectionHeading` (forgiving matching), `VaultDiagnostics` (heading/template detection), and `TemplaterRenderer`/`MomentFormat` (native rendering of Templater date/title tags). The bitmap render (`StrokeRenderer`), image I/O (`DiagramRepository`), the grid paint (`drawMonthGrid`), the `ScribbleIndexer` recognition + sidecar I/O, and the `VaultScanner` filesystem walk are thin Android-dependent glue, exercised manually on device.
